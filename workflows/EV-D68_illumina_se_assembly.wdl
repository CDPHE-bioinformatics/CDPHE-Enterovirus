...
workflow EVD68_illumina_se_assembly {
...
task seqyclean {
    ...
    command <<<
        ...
        seqyclean -minlen 25 -qual 30 30 -gz -U ~{fastq_1} -c ~{contam} -o ~{sample_name}_clean
...
task align_reads {
    ...
    command <<<
        ...
        bwa mem -t 2 reference.fasta ~{fastq_1} | \
        samtools sort | \
